...
rest_api_key = "38786035c3ef07af82be4950dc438535" 
...
lat = result['y']  # 위도
lon = result['x']  # 경도
emd = result['region_3depth_name'] # 읍면동
'''
############################################################
'''
# 주소 데이터 깔끔하게 다듬기
for i in range(len(mybuilding)):
    if mybuilding["도로명대지위치"].isnull()[i] == True:
        continue
    a = mybuilding["도로명대지위치"][i].split(' ')
    mybuilding["도로명대지위치"][i] = " ".join(a[0:4])
'''
################